...
export class PurchasesPage implements OnInit {
  purchases: any[] = [];
  constructor(private databaseService: DataBaseService,private platform:Platform,private alertify:AlertsAndNotificationsService,private dataProvider:DataProviderService,private detectorRef:ChangeDetectorRef) {}
  stocks: any[] = [];
  ...
  getWarehouse(){
    this.databaseService.getWarehouses().then((data) => {
      this.warehouses = []
  ...
  async export() {
    ...
    for (const e of this.purchases) {
      ...
      mainData.push(',,,,,,,,' +'Total,'+ entries.reduce((a, b) => a + b.items.length, 0));
  ...
  setWarehouse(selectedPurchase:any,warehouse:any){
    if (selectedPurchase){
      ...
      this.databaseService.updatePurchase(selectedPurchase.id,{warehouse:warehouse}).then(()=>{
